...
describe('sendEmail (Server Action)', () => {
  ...
  it('1. 入力値が不正な場合（Zodエラー）: エラーメッセージと安全なfieldsを返すこと', async () => {
  ...
  it('2. Turnstile トークンがない場合: チェック要求エラーを返すこと', async () => {
    const formData = new FormData();
  ...
  it('3. Turnstile 認証が失敗した場合: 認証エラーを返すこと', async () => {
  ...
  it('4. Resend からエラーが返ってきた場合: 送信失敗エラーを返すこと', async () => {
    global.fetch = jest.fn().mockResolvedValue({
  ...
  it('5. 全ての検証が成功した場合: 送信完了メッセージを返すこと', async () => {
    global.fetch = jest.fn().mockResolvedValue({
      json: jest.fn().mockResolvedValue({ success: true }),
    } as unknown as Response);
